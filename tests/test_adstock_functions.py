...
def test_adstock_shape():
    ...
    n_weeks = 20
    n_channels = 3
    x = np.random.uniform(0, 100, (n_weeks, n_channels))
...
def test_adstock_causal():
    ...
    n_weeks = 10
    n_channels = 1
    x = np.zeros((n_weeks, n_channels))
    x[5, 0] = 100  # Single impulse at t=5
...
def test_adstock_accumulates():
    ...
    n_weeks = 10
    n_channels = 1
    x = np.ones((n_weeks, n_channels)) * 10
    alpha = pt.as_tensor([0.5])
    ...
    assert np.all(result_val >= x - 1e-6)
